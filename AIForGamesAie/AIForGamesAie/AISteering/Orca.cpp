...
Orca::Orca(LevelState* level) : Agent::Agent(level)
{
	SetOrigin(27 / 2, 13 / 2);
	...
	AddComponent<Drawable>(new Drawable(m_level->GetResources()->GetTexture("spr_orca.png"), 27, 13));
...
void Orca::Update(float deltaTime)
{
...
void Orca::DrawInEditor(const Vector2& mousePos)
{
	m_position = mousePos;
	Agent::Draw();
